...
function Digiposte(opt) {
  var that = this;
  opt = opt || {};
  that.config = {baseUrl: process.env['DIGIPOSTE_API_BASE_URL'] || 'https://api.laposte.fr/digiposte/1.0'};
  _.extend(that, opt);
...
Digiposte.prototype.auth = function (opt, cb) {
  ...
  return that.apiRequest(
    ...
    .then(function (result) {
      that.username = opt.username || process.env['DIGIPOSTE_API_USERNAME'];
 ...
 * @TODO
 */
Digiposte.prototype.getTou = function (cb) {
  var that = this;
  return that.apiRequest(
    {
      uri: '/cgu',
 ...
 * @async
 * @param {Object} options Options object.
 * @param {String} options.location The location of the documents (NULL, SAFE, INBOX, or TRASH).
 ...
 *
 * The result is a Buffer populated with binary data of the downloaded thumbnail.
 *
 ...
 * @TODO
 */
Digiposte.prototype.getProfile = function (cb) {
  var that = this;
  return that.apiRequest(
    {
      uri: '/profile',
 ...
 *
 * The result is a Buffer populated with binary data of the downloaded avatar.
 *
 * @method getProfileAvatar
 * @async
 * @param {Function} [cb] The callback(err, result) function to be called when the request is fulfilled, if not defined a promise is returned.
 * @optional
 * @return {Promise} A promise, fulfilled when the request is done.
 ...
 * @TODO
 */
Digiposte.prototype.getProfileAvatar = function (cb) {
  var that = this;
  return that.apiRequest(
    {
      uri: '/profile/avatar',
